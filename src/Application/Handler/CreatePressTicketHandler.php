...
use MobilityWork\Application\Command\CreatePressTicketCommand;
...
final class CreatePressTicketHandler
{
    ...
    public function __invoke(CreatePressTicketCommand $command): TicketId
    {
        ...
        if ($command->media !== null) {
            $userData['user_fields'] = ['press_media' => $command->media];
        }
        ...
        $customFields = array_filter([
            CustomFieldId::TICKET_TYPE => TicketType::Press->value,
            CustomFieldId::HOTEL_ADDRESS => $command->city,
            CustomFieldId::LANGUAGE => $command->languageName,
        ]);

        $this->logger->info('Creating press ticket', ['email' => $command->email]);
